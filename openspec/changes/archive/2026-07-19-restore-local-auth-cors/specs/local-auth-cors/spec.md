...
The backend SHALL accept CORS preflight and registration requests from both
`http://localhost:5173` and `http://127.0.0.1:5173` for `/api/auth/**`.
...
The `app.cors.allowed-origins` configuration SHALL add allowed origins without
removing the standard local frontend origins.
